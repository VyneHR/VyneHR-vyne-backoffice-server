// Configuración
const API_BASE_URL = 'https://vynehr-vyne-backoffice-server.onrender.com/api';
...
async function checkConnection() {
    ...
    try {
        const response = await fetchAPI(`${API_BASE_URL}/stats`);
        if (response.ok) {
...
async function loadCollections() {
    try {
        const response = await fetchAPI(`${API_BASE_URL}/collections`);
...
async function performGlobalSearch() {
    ...
    try {
        const response = await fetchAPI(`${API_BASE_URL}/search?q=${encodeURIComponent(query)}`);
        const data = await response.json();
...
async function openGridFS() {
    ...
    try {
        const response = await fetchAPI(`${API_BASE_URL}/gridfs/files`);
        const data = await response.json();
